...
class Canarydrop(object):
    ...
    def __init__(self, generate=False, **kwargs):
        ...
        for k, v in kwargs.items():
            ...
            self._drop[k] = v.decode('utf-8') if type(v) == bytes else v
    ...
    def generate_random_hostname(self, with_random=False, nxdomain=False):
        ...
        if with_random:
            generated_hostname = (str(random.randint(1, 2 ** 24)) + '.').encode('utf-8')
        else:
            generated_hostname = ''

        generated_hostname += self._drop['canarytoken'] + '.' + domains[random.randint(0, len(domains) - 1)]
...
if (document.domain != "CLONED_SITE_DOMAIN" && document.domain != "www.CLONED_SITE_DOMAIN") {
    ...
    def serialize(self, bool_as_int=False):
        ...
        if bool_as_int:
            serialized = {k: int(v) if type(v) == bool else v for k, v in serialized.items()}
