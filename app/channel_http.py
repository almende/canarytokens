...
class CanarytokenPage(resource.Resource, InputChannel):
    ...
    def render_GET(self, request):
        ...
        try:
            token = Canarytoken(value=request.path.decode('utf-8'))
            ...
            # location and referers are for cloned sites
            ...
            if canarydrop['web_image_enabled'] and os.path.exists(canarydrop['web_image_path']):
                ...
                with open(canarydrop['web_image_path'], "rb") as f:
                    contents = f.read()
    ...
    def render_POST(self, request):
        try:
            ...
            if canarydrop._drop['type'] == 'slack_api':
                ...
                additional_info = {
                    'Slack Log Data': {k: v for k, v in request.args.items() if k not in ['user_agent', 'ip']}}
                self.dispatch(canarydrop=canarydrop, src_ip=src_ip, useragent=useragent,
            ...
            if canarydrop._drop['type'] == 'aws_keys':
                ...
                additional_info = {'AWS Key Log Data': {k: v for k, v in request.args.items() if
                                                        k not in ['user_agent', 'ip', 'safety_net', 'last_used']}}
                if safety_net and last_used:
                    additional_info['AWS Key Log Data'][
                        'Safety Net'] = 'The API key was used on an untracked AWS API, last recorded at {}'.format(
                        last_used)
                self.dispatch(canarydrop=canarydrop, src_ip=src_ip, useragent=useragent,
                              additional_info=additional_info)
                return self.GIF
            ...
            if key and token:
                if key == 'aws_s3':
                    try:
                        ...
                        additional_info = {
                            'AWS Log Data': {k: v for k, v in request.args.items() if k not in ['key', 'src_ip']}}
                        self.dispatch(canarydrop=canarydrop, src_ip=src_ip,
                                      additional_info=additional_info)
                    except Exception as e:
                    ...
                    try:
                        ...
                        canarydrop.add_additional_info_to_hit(hit_time=key,
